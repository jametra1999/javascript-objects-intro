...
Storing, organizing, and manipulating data is at the core of what makes a computer program do cool stuff. Programmers use different data structures to accomplish different goals. A data structure is a particular way of organizing data in a computer so that it can be used efficiently.

In JavaScript, the most common data structure is called an _object_. Objects are similar to safety deposit boxes. To access the contents of a safety deposit box, you need to get the correct key for that box. With an _object_ you can access a _value_ by using its _key_. 
...
2. Assign an object to the variable `tanniesRecipe`

3. The keys in this object should be sugar, butter, eggs, walnuts, secretIngredient1, secretIngredient2, secretIngredient3 and set the values to whatever you like. `Example: "1 cup", "2 pounds", "A lot", etc.`

4. Add a key to this object called `bakingInstructions` and set it equal to another object. Make the keys step1, step2, step3, step4, step5, and step6. Set the values to whatever you think the recipe requires :-)

5. Assign an object to the variable `playlist` — the keys will be artist names and the values will be song titles. 

6. Create a function `updatePlaylist` that accepts three parameters: the playlist (an object), an artist name (a string), and a song title. The body of the function should add the song and artist as a key-value pair to the playlist object. The function should return the whole playlist.

7. Create a function `removeFromPlaylist` that accepts two arguments (the playlist object and the artist name). The body of the function should delete the key-value pair from the playlist and return the updated playlist.
